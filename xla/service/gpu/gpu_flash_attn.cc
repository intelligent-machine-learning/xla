...
bool IsFwdCustomCallToFlashAttn(const HloInstruction &hlo) {
...
bool IsBwdCustomCallToFlashAttn(const HloInstruction &hlo) {
  if (hlo.opcode() != HloOpcode::kCustomCall) {
    return false;
  }
  const std::string &target = hlo.custom_call_target();
  return target == kGpuFlashAttnBwdCallTarget ||
...
absl::Status RunFlashAttnBwd(
  ...
  if (seqlen_q > 0) {
    ...
    // to 0.
    TF_RETURN_IF_ERROR(
        stream->MemZero(&grad_key_buffer, grad_key_buffer.size()));
    TF_RETURN_IF_ERROR(
        stream->MemZero(&grad_value_buffer, grad_value_buffer.size()));
    TF_RETURN_IF_ERROR(
