...
absl::Status FlashAttnFwdThunk::ExecuteOnStream(const ExecuteParams& params) {
  ...
  TF_RETURN_IF_ERROR(RunFlashAttnFwd(
      ...
      cu_seqlens_query_buffer, cu_seqlens_key_buffer, output_buffer,
      softmax_lse_buffer, s_dmask_buffer, rng_state_buffer, alibi_slopes_buffer,
      softmax_lse_accum_buffer, output_accum_buffer, -1, -1));
...
FlashAttnBwdThunk::FlashAttnBwdThunk(
    ...
    BufferAllocation::Slice query_slice, BufferAllocation::Slice key_slice,
    BufferAllocation::Slice value_slice,
    BufferAllocation::Slice cu_seqlens_query_slice,
    BufferAllocation::Slice cu_seqlens_key_slice,
    BufferAllocation::Slice output_slice,
    BufferAllocation::Slice softmax_lse_slice,
    BufferAllocation::Slice rng_state_slice,
...
absl::Status FlashAttnBwdThunk::ExecuteOnStream(const ExecuteParams& params) {
  ...
  TF_RETURN_IF_ERROR(RunFlashAttnBwd(
      ...
      value_buffer, cu_seqlens_query_buffer, cu_seqlens_key_buffer,
      output_buffer, softmax_lse_buffer, rng_state_buffer, grad_query_buffer,
      grad_key_buffer, grad_value_buffer, grad_softmax_buffer,
      alibi_slopes_buffer, grad_query_accum_buffer, -1, -1));
